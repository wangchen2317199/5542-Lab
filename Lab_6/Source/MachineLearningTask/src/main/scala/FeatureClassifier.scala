...
/**
  ...
  */
object FeatureClassifier {
  def main(args: Array[String]) {
    System.setProperty("hadoop.home.dir", "C:\\winutils")
    val IMAGE_CATEGORIES = Array("Fork", "Spoon", "Knife")
    val sparkConf = new SparkConf().setAppName("FeatureClassification").setMaster("local[*]")
    val sc = new SparkContext(sparkConf)
    val train = sc.textFile("trainingData")
    val test = sc.textFile("testData")
